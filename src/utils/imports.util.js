...
exports.ImportsUtil = void 0;
const path = require("path");
const ts_morph_1 = require("ts-morph");
const file_engine_1 = require("../app/engines/file.engine");
const ast = new ts_morph_1.Project();
class ImportsUtil {
    constructor() { }
    static getInstance() {
     ...
     * @param variableValue
     */
    findInEnums(srcFile, variableName, variableValue) {
        let res = '';
        srcFile.getEnum(e => {
     ...
     * @param variableValue
     */
    findInClasses(srcFile, variableName, variableValue) {
        let res = '';
        srcFile.getClass(c => {
            let staticProperty = c.getStaticProperty(variableValue);
            if (staticProperty) {
     ...
     */
    findInObjectVariableDeclaration(variableDeclaration, variablesAttributes) {
        let variableKind = variableDeclaration.getKind();
        if (variableKind && variableKind === ts_morph_1.SyntaxKind.VariableDeclaration) {
            let initializer = variableDeclaration.getInitializer();
            if (initializer) {
                let initializerKind = initializer.getKind();
                if (initializerKind && initializerKind === ts_morph_1.SyntaxKind.ObjectLiteralExpression) {
                    let compilerNode = initializer.compilerNode, finalValue = '';
     ...
     */
    findValueInImportOrLocalVariables(inputVariableName, sourceFile, decoratorType) {
        let metadataVariableName = inputVariableName, searchedImport, aliasOriginalName = '', foundWithNamedImport = false, foundWithDefaultImport = false, foundWithAlias = false;
        const file = typeof ast.getSourceFile(sourceFile.fileName) !== 'undefined'
         ...
         */
        imports.forEach(i => {
            let namedImports = i.getNamedImports(), namedImportsLength = namedImports.length, j = 0;
            if (namedImportsLength > 0) {
                for (j; j < namedImportsLength; j++) {
                    let importName = namedImports[j].getNameNode().getText(), importAlias;
        ...
        function hasFoundValues(variableDeclaration) {
            ...
            if (variableKind && variableKind === ts_morph_1.SyntaxKind.VariableDeclaration) {
                ...
                if (initializer) {
                    let initializerKind = initializer.getKind();
                    if (initializerKind && initializerKind === ts_morph_1.SyntaxKind.ObjectLiteralExpression) {
                        let compilerNode = initializer.compilerNode;
        ...
        if (typeof searchedImport !== 'undefined') {
            ...
            if (typeof importPathReference !== 'undefined') {
                ...
                const sourceFileImport = typeof ast.getSourceFile(importPath) !== 'undefined'
                ...
                if (sourceFileImport) {
                    ...
                    if (variableDeclaration) {
                        ...
                        if (exportDeclarations && exportDeclarations.size > 0) {
                            for (const [exportDeclarationKey, exportDeclarationValues] of exportDeclarations) {
                                exportDeclarationValues.forEach(exportDeclarationValue => {
                                    if (exportDeclarationValue instanceof ts_morph_1.VariableDeclaration &&
            ...
            if (!importPathReference &&
                ...
                const originalSourceFilePathFolder = originalSourceFilePath.substring(0, originalSourceFilePath.lastIndexOf('/'));
                const finalImportedPath = originalSourceFilePathFolder + '/' + searchedImport.getModuleSpecifierValue();
                const finalImportedPathData = file_engine_1.default.getSync(finalImportedPath);
            ...
            if (variableDeclaration) {
                ...
                if (variableKind && variableKind === ts_morph_1.SyntaxKind.VariableDeclaration) {
                    ...
                    if (initializer) {
                        let initializerKind = initializer.getKind();
                        if (initializerKind &&
                            initializerKind === ts_morph_1.SyntaxKind.ObjectLiteralExpression) {
                            let compilerNode = initializer.compilerNode;
                            return compilerNode.properties;
                        }
                        else if (initializerKind &&
                            (initializerKind === ts_morph_1.SyntaxKind.StringLiteral ||
                                initializerKind === ts_morph_1.SyntaxKind.NoSubstitutionTemplateLiteral)) {
    ...
    getFileNameOfImport(variableName, sourceFile) {
        const file = typeof ast.getSourceFile(sourceFile.fileName) !== 'undefined'
            ? ast.getSourceFile(sourceFile.fileName)
            : ast.addSourceFileAtPath(sourceFile.fileName); // tslint:disable-line
        const imports = file.getImportDeclarations();
        let searchedImport, aliasOriginalName = '', finalPath = '', foundWithAlias = false;
        imports.forEach(i => {
            let namedImports = i.getNamedImports(), namedImportsLength = namedImports.length, j = 0;
            if (namedImportsLength > 0) {
                for (j; j < namedImportsLength; j++) {
        ...
        if (typeof searchedImport !== 'undefined') {
            let importPath = path.resolve(path.dirname(sourceFile.fileName) +
                '/' +
     ...
     */
    findFilePathOfImportedVariable(inputVariableName, sourceFilePath) {
        let searchedImport, finalPath = '', aliasOriginalName = '', foundWithAlias = false;
        const file = typeof ast.getSourceFile(sourceFilePath) !== 'undefined'
     ...
     */
    findPropertyValueInImportOrLocalVariables(inputVariableName, sourceFile) {
        let variablesAttributes = inputVariableName.split('.'), metadataVariableName = variablesAttributes[0], searchedImport, aliasOriginalName = '', foundWithAlias = false;
        const file = typeof ast.getSourceFile(sourceFile.fileName) !== 'undefined'
            ? ast.getSourceFile(sourceFile.fileName)
            : ast.addSourceFileAtPath(sourceFile.fileName); // tslint:disable-line
        const imports = file.getImportDeclarations();
