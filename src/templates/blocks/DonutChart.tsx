...
const SIZE_CONFIG: Record<
    DonutSize,
    { viewBox: number; radius: number; strokeWidth: number; cssClass: string; pctClass: string }
> = {
    sm: {
        viewBox: 80,
        radius: 34,
        strokeWidth: 7,
        cssClass: 'cdx-overview-donut',
        pctClass: 'cdx-overview-donut-pct'
    },
    lg: {
        viewBox: 120,
        radius: 50,
        strokeWidth: 10,
        cssClass: 'cdx-coverage-donut',
        pctClass: 'cdx-coverage-donut-pct'
    }
};
    ...
    const segment = (len: number, offset: number, color: string, label: string): string => {
        ...
        return (
            <circle
                ...
                fill="none"
                stroke={color}
                stroke-width={String(cfg.strokeWidth)}
    ...
    return (
        ...
        >
            ...
            {segment(
                ...
                docOffset,
                COLORS.green,
                `Documented: ${documented}${size === 'lg' ? ` (${Math.round(docFrac * 100)}%)` : ''}`
            )}
            {segment(
                partLen,
                partOffset,
                COLORS.yellow,
                `Partial: ${partial}${size === 'lg' ? ` (${Math.round(partFrac * 100)}%)` : ''}`
            )}
            {segment(
                undocLen,
                undocOffset,
                COLORS.red,
                `Undocumented: ${undocumented}${size === 'lg' ? ` (${Math.round((total > 0 ? undocumented / total : 0) * 100)}%)` : ''}`
            )}
            <text
                x={String(cx)}
                y={String(cx)}
