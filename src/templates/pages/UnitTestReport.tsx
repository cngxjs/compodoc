...
const coveragePctClass = (pct: number): string => {
    if (pct <= 33) return 'cdx-coverage-pct--low';
    if (pct <= 66) return 'cdx-coverage-pct--medium';
    return 'cdx-coverage-pct--high';
};

const filePct = (f: UnitTestFile): number =>
    Math.round((f.statements.coveragePercent + f.branches.coveragePercent + f.functions.coveragePercent + f.lines.coveragePercent) / 4);

const fileCovCount = (f: UnitTestFile): string => {
    const covered = (f.statements.covered ?? 0) + (f.branches.covered ?? 0) + (f.functions.covered ?? 0) + (f.lines.covered ?? 0);
    const total = (f.statements.total ?? 0) + (f.branches.total ?? 0) + (f.functions.total ?? 0) + (f.lines.total ?? 0);
...
const fileLink = (f: UnitTestFile): string | null => {
    ...
    if (f.linktype === 'entity') return `./entities/${f.name}.html`;
    return `./${f.linktype}s/${f.name}.html`;
};

const fileGroupKey = (f: UnitTestFile): string => {
    if (f.linksubtype) return f.linksubtype;
    if (f.linktype) return f.linktype;
    return '_other';
};

const ChevronIcon = (): string => (
    <svg class="cdx-coverage-group-chevron" viewBox="0 0 16 16" fill="none"
...
export const UnitTestReport = (props: UnitTestReportProps): string => {
    ...
    const overallPct = Math.round(((stmts.coveragePercent ?? 0) + (branches.coveragePercent ?? 0) + (funcs.coveragePercent ?? 0) + (lines.coveragePercent ?? 0)) / 4);
    const totalItems = (stmts.total ?? 0) + (branches.total ?? 0) + (funcs.total ?? 0) + (lines.total ?? 0);
    const coveredItems = (stmts.covered ?? 0) + (branches.covered ?? 0) + (funcs.covered ?? 0) + (lines.covered ?? 0);
    ...
    return (<>
        ...
        {CoverageSummary({
            donutHtml: DonutChart({ percent: overallPct, documented: coveredItems, partial: 0, undocumented: uncoveredItems, total: totalItems, size: 'lg' }),
            stats: [
                { value: `${stmts.coveragePercent ?? 0}%`, label: t('statements'), subtitle: `${stmts.covered ?? 0}/${stmts.total ?? 0}`, modifier: (stmts.coveragePercent ?? 0) > 66 ? 'documented' : (stmts.coveragePercent ?? 0) > 33 ? 'partial' : 'undocumented' },
                { value: `${branches.coveragePercent ?? 0}%`, label: t('branches'), subtitle: `${branches.covered ?? 0}/${branches.total ?? 0}`, modifier: (branches.coveragePercent ?? 0) > 66 ? 'documented' : (branches.coveragePercent ?? 0) > 33 ? 'partial' : 'undocumented' },
                { value: `${funcs.coveragePercent ?? 0}%`, label: t('functions'), subtitle: `${funcs.covered ?? 0}/${funcs.total ?? 0}`, modifier: (funcs.coveragePercent ?? 0) > 66 ? 'documented' : (funcs.coveragePercent ?? 0) > 33 ? 'partial' : 'undocumented' },
                { value: `${lines.coveragePercent ?? 0}%`, label: t('lines'), subtitle: `${lines.covered ?? 0}/${lines.total ?? 0}`, modifier: (lines.coveragePercent ?? 0) > 66 ? 'documented' : (lines.coveragePercent ?? 0) > 33 ? 'partial' : 'undocumented' },
            ],
        })}

        <div class="cdx-ut-file-stats">
            <span class="cdx-ut-file-stat">{validFiles.length} files</span>
            <span class="cdx-ut-file-stat cdx-ut-file-stat--good">{fullyCovered} fully covered</span>
            <span class="cdx-ut-file-stat cdx-ut-file-stat--warn">{partialFiles} partial</span>
            {uncoveredFiles > 0 && <span class="cdx-ut-file-stat cdx-ut-file-stat--bad">{uncoveredFiles} uncovered</span>}
        </div>

        <div class="cdx-coverage-filter">
            <input type="text" class="cdx-coverage-filter-input"
                placeholder="Filter files..." aria-label="Filter unit test files"
                data-cdx-coverage-filter />
            <button type="button" class="cdx-coverage-filter-clear"
                aria-label="Clear filter" data-cdx-coverage-filter-clear>&times;</button>
        </div>
        ...
        {hasGroups ? sortedKeys.map(typeKey => {
            ...
            return (
                <details open class="cdx-coverage-group" data-cdx-coverage-group={typeKey}>
                    <summary class="cdx-coverage-group-header">
                        ...
                        <span class="cdx-coverage-group-fraction">{groupFullyCovered}/{groupFiles.length}</span>
                    </summary>
                    ...
                    <table class="cdx-coverage-table">
                        <thead>
                            <tr>
                                ...
                                <th data-cdx-sort="file" aria-sort="none" class="cdx-coverage-file-col">
                        ...
                        <tbody>
                            {groupFiles.map(f => {
                                const pct = filePct(f);
                                return (
                                    <tr data-cdx-coverage-name={(f.name ?? '').toLowerCase()}
                                        data-cdx-coverage-file={f.filePath.toLowerCase()}
                                        data-cdx-coverage-pct={String(pct)}>
                                        <td>{fileLink(f) ? <a href={fileLink(f)!}>{f.name}</a> : f.name}</td>
                                        <td class="cdx-coverage-file cdx-coverage-file-col" title={f.filePath}>
                                            {shortPath(f.filePath)}
                                        </td>
                                        <td class="cdx-coverage-cell">
                                            <span class={`cdx-coverage-pct ${coveragePctClass(pct)}`}>{pct}%</span>
                                            <span class="cdx-coverage-count-detail">({fileCovCount(f)})</span>
                                        </td>
                                    </tr>
                                );
                            })}
            ...
            <table class="cdx-coverage-table">
                ...
                <tbody>
                    {validFiles.sort((a, b) => filePct(a) - filePct(b)).map(f => {
                        const pct = filePct(f);
                        return (
                            <tr data-cdx-coverage-name={(f.name ?? '').toLowerCase()}
                                data-cdx-coverage-file={f.filePath.toLowerCase()}
                                data-cdx-coverage-pct={String(pct)}>
                                <td class="cdx-coverage-file" title={f.filePath}>{shortPath(f.filePath)}</td>
                                <td class="cdx-coverage-cell">
                                    <span class={`cdx-coverage-pct ${coveragePctClass(pct)}`}>{pct}%</span>
                                    <span class="cdx-coverage-count-detail">({fileCovCount(f)})</span>
                                </td>
