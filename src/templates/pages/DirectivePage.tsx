...
const DirectiveMetadata = (directive: any): string => {
    ...
    if (directive.standalone) {
        rows.push(MetadataCodeRow(t('standalone'), String(directive.standalone)));
    }
    ...
    if (directive.providers) {
        rows.push(
            MetadataCodeRow(t('providers'), directive.providers.map((p: any) => p.name).join(', '))
        );
    }
    ...
    if (hasExtends) {
        rows.push(
            MetadataRow(
                'extends',
                (directive.extends as string[]).map(ext => linkTypeHtml(ext)).join(' ')
            )
        );
    }
    if (hasImplements) {
        rows.push(
            MetadataRow(
                'implements',
                (directive.implements as string[]).map(impl => linkTypeHtml(impl)).join(' ')
            )
        );
    }
