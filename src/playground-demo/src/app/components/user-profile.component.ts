...
export class UserProfileComponent implements OnInit, OnDestroy {
    ...
    enterEditMode(): void {
        ...
        setTimeout(() => {
            const firstNameInput = this.profileContainer.nativeElement.querySelector(
    ...
    async saveProfile(): Promise<void> {
        ...
        try {
            ...
            const updatedUser = await this.userService
                .updateUser(this.user.id, updatedUserData)
                .toPromise();
    ...
    async changeStatus(newStatus: UserStatus): Promise<void> {
        ...
        try {
            const updatedUser = await this.userService
                .changeUserStatus(this.user.id, newStatus)
                .toPromise();
    ...
    private getChangedFields(formValue: any): string[] {
        ...
        if (this.originalUserData.address) {
            Object.keys(formValue.address).forEach(field => {
                if (
                    formValue.address[field] !==
                    this.originalUserData!.address![
                        field as keyof typeof this.originalUserData.address
                    ]
                ) {
        ...
        if (this.originalUserData.preferences) {
            Object.keys(formValue.preferences).forEach(field => {
                if (
                    formValue.preferences[field] !==
                    this.originalUserData!.preferences![
                        field as keyof typeof this.originalUserData.preferences
                    ]
                ) {
    ...
    getContainerClasses(): Record<string, boolean> {
        return {
            ...
            ...(this.config.customCssClasses?.reduce(
                (acc, className) => {
                    acc[className] = true;
                    return acc;
                },
