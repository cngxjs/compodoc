...
| menuLayout | _config-only_ | `'type' \| 'feature'` | `'type'` | Whole-sidebar layout. `'type'` keeps the default per-kind chapters (Components / Directives / Injectables / ...). `'feature'` splits the cross-kind sidebar into two chapters: **Features** (components, directives, pipes, injectables, classes, guards, interceptors, entities) and **References** (interfaces, functions, typealiases, variables, enumerations). Both share the same folder / `@category` bucket paths, so a `button/` folder may appear in both chapters when it contains both kinds. `@category` overrides folder keys. Modules / Routes / Additional Pages stay top-level in both modes; Miscellaneous is suppressed under `'feature'` (everything moves into References). Folder depth is controlled by `groupDepth` (use `>= 2` in multi-project workspaces). `toggleMenuItems` accepts the keys `features` and `references`. Under `'feature'`, every bucket also gets an auto-generated landing page at `categories/<bucket-id>.html` (v0.6.0+) - leaf and intermediate folders alike - linked from the sidebar bucket label; the chevron continues to toggle expand |
...
| referencesName | _config-only_ | string | `''` (i18n default) | Override the **References** chapter heading under `menuLayout: 'feature'`. Empty falls back to the translated `references` i18n key |
...
| `@wcag <level>` | Renders a colored WCAG-conformance chip in the entity hero. Accepts `A`, `AA`, or `AAA` (case-insensitive); other values log a build-time warning and fall back to no chip. Also surfaces as a `data-pagefind-filter-wcag` attribute so the search-palette WCAG facet can narrow to a specific level |
| `@a11y <markdown>` | Free-form accessibility note rendered as an "Accessibility" section above the description on the Info tab. Markdown-formatted - embed links to WAI-ARIA APG patterns, code samples, or short notes about announced messages and keyboard behaviour |
