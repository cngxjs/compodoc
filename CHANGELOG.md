...
- **i18n keys.** `references` and `referenced-by` added in en-US + de-DE; other 15 locales fall back to the literal key per the existing `I18nEngine.exists(key) ? translate : key` semantics. Configure `featuresName` / `referencesName` for non-English builds without touching translations.
- **New override-context name.** `referenced-by` joins the block-level override list (`src/migrate/override-names.ts`); the parity drift spec re-derives it from disk.
